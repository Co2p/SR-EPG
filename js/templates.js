...
const programDetailTemplate = Handlebars.compile('<detail>{{#if imageurl}}<img alt="{{title}}" src="{{imageurl}}"/>{{/if}}{{#if listenpodfile.url}}<div>{{listenpodfile.url}}</div>{{/if}}<p>{{title}}</p><description>{{description}}</description><text>{{text}}</text></detail>')
