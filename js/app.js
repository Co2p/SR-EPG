...
function build(channel) {
  console.time('build ' + channel.name);
  getJSON(channel.scheduleurl + "&format=json&pagination=false").then((data) => {
    ...
    for (let j in api.schedule) {
      ...
      program.programcolor = colorGenerator(channel.id, color);
  ...
  console.timeEnd('build ' + channel.name);
}
  ...
  if (modalAnimationDone) {
    ...
    if ($('.program-info').length > 0) {
      ...
      if (programParent.hasClass('fadedprogram')) {
        programParent.animate({opacity: 0.5}, 600);
      }
      programDetail.hide();
      ...
      try {
        gaSend('Episode detail', 'expand', program.find('p')[0].textContent);
      } catch (e) {
      ...
      if (programDetail.length == 0) {
        getProgramData(program, $(e.parentElement).attr('eid'), $(e.parentElement).attr('pid'));
      }
    }
  }
}

function getProgramData(program, eid, pid) {
  let programDetail = program.find('detail');
  getJSON(episodeURL({'id': eid})).then(function (data) {
    ...
    getJSON(programURL({'id': pid})).then(function (data) {
