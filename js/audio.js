...
$('div').on('click', (e) => {
    if (e.target.className == "playing") {
        ...
        play(liveAudioURL({id: streamid}), streamid, player.parentElement.parentElement.title + ' - Live');
    }
});

function play(stream, streamid, title = 'Playing') {
    if (playing != streamid) {
        if (playing != 0) {
            pause();
        }
...
function pause() {
    let audio = $('audio');
    $('nav').find('h').text('Sveriges Radio EPG');
    audio[0].pause();
    audio[0].src = '#'; //stops stream
    playing = 0;
}

$('nav').find('h').on('click', (e) => {
    if (playing != 0) {
        expandProgram($('*[eid="' + playing + '"]')[0]);
    }
});
